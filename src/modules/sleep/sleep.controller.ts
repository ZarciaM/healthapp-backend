...
export const create = asyncHandler(async (req: Request, res: Response) => {
  const entry = await sleepService.createEntry(req.user!.userId, req.body);

  sendSuccess(res, 201, "Entrée de sommeil créée", { entry });
});
...
export const getLatest = asyncHandler(async (req: Request, res: Response) => {
  const entry = await sleepService.getLatest(req.user!.userId);

  sendSuccess(res, 200, "Dernière entrée de sommeil", { entry });
});
...
export const getAverages = asyncHandler(async (req: Request, res: Response) => {
  ...
  const averages = await sleepService.getAverages(req.user!.userId, days);

  sendSuccess(res, 200, "Moyennes de sommeil", { averages });
});

export const remove = asyncHandler(async (req: Request, res: Response) => {
  await sleepService.deleteEntry(req.user!.userId, req.params.id as string);

  sendSuccess(res, 200, "Entrée de sommeil supprimée");
});
