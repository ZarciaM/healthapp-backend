...
export function startScheduler(): ScheduledTask {
  ...
  const task = cron.schedule("* * * * *", async () => {
    ...
    try {
      const medicationReminders = await findDueMedicationReminders(now);

      if (medicationReminders.length > 0) {
        logger.info(`Found ${medicationReminders.length} due medication reminder(s)`);
    ...
    // --- Hydration reminders ---
    try {
      const currentTime = formatUTCHHMM(now);
      const hydrationReminders = await findDueHydrationReminders(currentTime);

      if (hydrationReminders.length > 0) {
        logger.info(`Found ${hydrationReminders.length} due hydration reminder(s)`);
